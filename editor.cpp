...
int main(int argc, char ** argv) {
  ...
  else b.push_back(row);
  try {
    vector < int > row;
    ifstream ifs(src);
    string cont((istreambuf_iterator < char > (ifs)),
  ...
  while (TRUE) {
    ...
    move(R, 0);
    if (msg == "")
      for (int i = 0; i < stat.length(); i++) addch(stat[i]);
    else {
      for (int i = 0; i < msg.length(); i++) addch(msg[i]);
      msg = "";
    }
    clrtoeol();
    curs_set(0);
    ...
    if (mod == "n") {
      if (ch == 'i') {
        ...
        if (ch == 'd')
          for (int i = 0; i < (cnt.length() ? times : 1); i++) {
            if (b.size() > 1 && r < b.size()) b.erase(b.begin() + r);
            if (r == b.size()) r--;
          }
        cnt = "";
        ...
        case '$':
          ...
          break;
        case 'x':
          if (b[r].size()) b[r].erase(b[r].begin() + c);
          break;
        case 'h':
          c ? c-- : c;
          break;
